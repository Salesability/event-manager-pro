...
describe('computePickedTotals (0062 picker)', () => {
  ...
  describe('province-rate tax', () => {
    it('computes tax as subtotal × ratePct/100', () => {
      ...
      expect(r.tax).toBe(130); // ON 13%
      expect(r.total).toBe(1130);
    });

    it('handles QC 14.975% with cent rounding', () => {
      const r = computePickedTotals([pick({ qty: 1, unitPrice: 1000 })], { ratePct: 14.975 });
      expect(r.tax).toBe(149.75);
