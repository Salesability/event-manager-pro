...
- `docs/designs/YYYY-MM-DD-kebab-slug/` — folder per chunk of work. Dates not numbers; solo dev, no PRs. Use start-date; same-day collisions get an `a`/`b` suffix.
- No separate `docs/designs/adr/`. If a folder contains a genuine architecture decision, write it as `decision.md` inside that folder.
