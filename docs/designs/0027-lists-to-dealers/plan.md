...
|----------|---------------------|-----------------|
| `src/app/(app)/lists/page.tsx` (rewrite) | `src/app/(app)/admin/people/page.tsx:1` | Same layer (admin server-component page that loads + renders one client admin component); same `await requireRole(...)` + parallel `loadX()` shape. |
...
2. **Route: keep `/lists` or move to `/dealers`?**
   - Keep `/lists`: zero-disruption, nav label already shifting from "Manage Lists" → "Dealers".
   - Move to `/dealers`: better URL hygiene; redirect `/lists` → `/dealers` for any bookmarks; touches middleware patterns.
   - **Working assumption:** keep `/lists` — pure URL cosmetics, defer until a `/dealers` aliasing is genuinely needed (e.g. when marketing-site links land).
...
- [ ] `src/app/(app)/lists/page.tsx`: drop the `<h1>Manage Lists</h1>` + the four-sentence subtitle linking to `/admin/people`.
- [ ] `src/app/(app)/lists/page.tsx`: drop the `ListCard` wrapper + the `🏢 Dealerships 26` header — the page *is* the dealers page; the section header is redundant.
- [ ] `src/app/(app)/lists/page.tsx`: rename the page header to a plain `<h1 className="font-display text-3xl text-navy">Dealers</h1>` (matches `/admin/people`'s style on `admin/people/page.tsx:17`).
- [ ] Delete the `ListCard` + `EmptyState` helper components from the file (only used by this page).
- [ ] `src/components/app/app-nav.tsx:11`: change `label: 'Manage Lists'` → `label: 'Dealers'`. Leave `href: '/lists'` (Open Question 2).
- [ ] Update the inline comment at the top of the page — the "Sales Coaches retired in 0020" preamble can shrink to a one-liner pointing at the new structure.
- [ ] `pnpm tsc --noEmit && pnpm lint` clean.

### Phase 2: TanStack DataTable foundation on Dealers

Goal: replace the bare `<ul>`-of-`<li>` with the same DataTable that `/admin/people` uses. Sortable, searchable, paginated.

- [ ] Create `src/features/dealers/dealers-columns.tsx` — anchor on `src/features/people/people-columns.tsx:54`. Columns: `name` (sortable), `contact` (composed `firstName + lastName`), `email` (sortable), `phone`, `address`, `actions`.
- [ ] Create `src/features/dealers/dealers-admin.tsx` — anchor on `src/features/people/people-admin.tsx:108`. Owns: `globalFilter` state, `archive(dealer)` server-action wiring, `Dialog.Root` for Add/Edit, `<DataTable>` invocation with `initialSorting=[{id:'name',desc:false}]`.
- [ ] Toolbar: `<input type="search">` (placeholder "Search by name, contact, or email…") + `+ Add Dealer` button on the right. Mirror `people-admin.tsx:183`.
- [ ] Cross-column global filter fn in `dealers-admin.tsx` matching the `peopleGlobalFilterFn` shape (`people-admin.tsx:24`).
- [ ] `src/app/(app)/lists/page.tsx` becomes a thin server component: `await requireRole(...)` (verify which role gates this — likely keep the layout-level gate), `loadDealers()`, render `<DealersAdmin dealers={...} />`.
- [ ] Empty state with `Clear filters` action when filtered → no rows; otherwise plain "No dealers yet."
- [ ] `pnpm tsc --noEmit && pnpm lint && pnpm test --run` clean.

### Phase 3: Radix Dialog + DealerForm rewrite

Goal: replace the current `app/(app)/lists/dealer-form.tsx` (custom `Field` helper + bare buttons) with a PersonForm-shaped form that uses the shared `Dialog` wrapper, the shared input class, and the shared toast wiring.

- [ ] Create `src/features/dealers/dealer-form.tsx` — anchor on `PersonForm` at `src/features/people/people-admin.tsx:324`. Same `useActionState<State, FormData>` shape, same `useEffect` toast/router-refresh wiring, same `Cancel` + submit footer.
...
- [ ] Delete `src/app/(app)/lists/list-actions.tsx` (its `AddDealerButton` + `DealerRowActions` move into `dealers-admin.tsx` as inline behavior, just like `/admin/people`).
...
- [ ] `pnpm tsc --noEmit && pnpm lint` clean.
- [ ] Smoke (web-test): `goto /lists`; expect h1 "Dealers" + search input + button "+ Add Dealer"; column headers `Name` / `Contact` / `Email` / `Phone` / `Address`; at least one row.
...
- [ ] Visual smoke (manual): screenshot the new `/lists` page; compare against the screenshot the user shared to confirm chrome reduction.
