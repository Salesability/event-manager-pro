...
**Depends on `0018-user-system`** — the role taxonomy (admin / coach / etc.), the `requireAdmin()` helper, `loadCurrentMembership()`, and the `app_metadata.role` claim are all foundations this plan reads. **0018 must ship Phase 1 + Phase 2 before 0019 can start Phase 1.** 0019 stays parked until then.
