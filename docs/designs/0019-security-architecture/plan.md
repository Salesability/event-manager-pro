...
| 4: `audit_log` table + `recordAudit()` helper + wire into sensitive actions | Done | - |
...
- [x] `src/lib/db/schema/audit-log.ts` — table with `id`, `occurredAt`, `actorUserId`, `actorRole` (denormalised at write time), `action` (enum: `user.role_changed | user.deactivated | dealer.archived | campaign.cancelled`), `targetTable` (text), `targetId` (bigint, nullable), `payload` (jsonb). No `archivable` mixin — append-only by convention. Three indexes: `actor_user_id`, `(action, occurred_at)`, `(target_table, target_id)`. **`coach.archived` dropped from the enum:** after the 0020 unification, "archive a coach" is just `archivePerson` with one of the facets being a coach role; emitting `user.deactivated` covers it without a separate event.
- [x] `pnpm db:generate` → review → committed as `drizzle/0004_worthless_titanium_man.sql`. **RLS hand-appended to the same file** (Drizzle doesn't model RLS — convention from `db-conventions`): `audit_log_service_role_all` (FOR ALL) and `audit_log_authenticated_read_own` (FOR SELECT, `actor_user_id = auth.uid()`). No anon policy → default-deny. Drizzle's `postgres` connection bypasses both via BYPASSRLS, so the writer path is unaffected. **One migration runner gotcha:** the journal `when` ordering matters; bumped 0004's `when` to `1778500800000` (after 0003's `1778414400000`) for the migrator to apply it.
- [x] `src/features/audit/actions.ts:recordAudit({ action, targetTable, targetId, payload })` — `'server-only'` import; reads current user via `getUser()`; throws on no-user (wired-it-wrong assertion since every caller is downstream of `requireRole(...)`); writes a single row with `actorRole` denormalised from `app_metadata.role`. **The DB insert itself is best-effort: wrapped in try/catch + `console.error` (in-eval fix for Codex High).** This deviates from Decision #3's "same transaction" framing — see eval-2026-05-06-1046 for rationale. The actions span DB + Supabase auth boundaries already, so wrapping audit in the mutation tx wouldn't actually keep them atomic; rejecting an action whose mutation has already committed would be worse than logging an audit gap.
- [x] Wire `recordAudit()` into: ~~`setUserRoles`, `deactivateUser` (0018 P1); `archiveCoach`~~ (retired in 0020 — collapsed into `updatePerson`/`archivePerson`); **`archivePerson` → `user.deactivated`**; **`updatePerson` (when roles changed pre vs post) → `user.role_changed`** with `payload: { before, after }` (sorted role arrays for stable diffs); **`archiveDealer` → `dealer.archived`** (now `.returning()`-gated so an archived-already noop doesn't emit a duplicate audit row); **`cancelCampaign` → `campaign.cancelled`** (only on the conditional UPDATE that actually flipped the status). **Two more emissions added in-eval (Codex Mediums):** `createPerson` → `user.role_changed` with `payload: { before: [], after: roles }` whenever a non-empty role set is granted at creation; `updatePerson` on→off auth ban → `user.deactivated` with `payload: { authUserId, via: 'updatePerson' }` after the ban succeeds. The `updatePerson` role-snapshot SELECT was also moved INSIDE the `db.transaction` block to close the race against a concurrent `updatePerson` between snapshot and `syncTeamMemberRoles` (Codex Medium #4).
- [x] Vitest test for `recordAudit()` happy path + the 5 wired actions emit rows of the right shape. **Done.** `src/features/audit/actions.test.ts` covers `recordAudit` itself (5 cases: happy path, missing app_metadata.role, no-user assertion, default null payload, null targetId). Wiring assertions added to `src/features/people/actions.test.ts`: `archivePerson` emits `user.deactivated`, `updatePerson` on→off emits `user.deactivated` plus role-diff emit, `createPerson` end-to-end emits `user.role_changed` with `before:[]` (post in-eval). `archiveDealer` + `cancelCampaign` wiring is type-checked + reachable via the existing schedule actions; not separately asserted (no schedule actions test file yet — would require a new fixture).
