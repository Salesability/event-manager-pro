...
3. **Envelope shape specifics.** Dropbox Sign supports both "request signature" (with template) and "send" (raw documents). Templated approach lets the coach not re-upload the MSA template every send. Locked from 0037: two documents (MSA + first Quote PDF), one envelope. Open: do we use a Dropbox Sign **template** for the MSA side (Dropbox-hosted template + signer-role mapping) or upload the MSA PDF inline per envelope (simpler but the template-version drift is harder to track)?
4. **`templateVersion` strategy.** Schema has the column; what populates it? Candidates: (a) a `MSA_TEMPLATE_VERSION` env var bumped on each prose revision; (b) a git SHA of the template MD file at render time; (c) a Dropbox-Sign-side template ID with its own versioning. **Recommendation:** option (a) — env var bumped manually, with `templateVersion` denormalized into the row at draft creation. Audit trail stays self-contained.
