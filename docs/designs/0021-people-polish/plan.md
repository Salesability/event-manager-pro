...
- [ ] `src/features/people/people-admin.tsx:PersonForm` — pre-submit guard: when `mode === 'edit' && person?.hasAppAccess && !wantsAppAccess` (i.e. existing person with app access, but the admin un-ticked all role boxes), run `confirm("Removing all roles from {name} will end their app access and ban their sign-in account. The contact record stays. Continue?")`; abort on cancel. Trigger changed from "App access toggle off" to "all roles unticked" since Phase 2 dropped the App access checkbox.
- [ ] `people-columns.tsx:actions` cell `onArchive` — build the confirm message from facets: e.g. `This archive will: drop coach role, end relationship with Trevors Nissan (customer), and ban sign-in for adam.godin@gmail.com. The contact record stays.` Use `person.roles` + `person.dealerLinks` + `person.hasAppAccess` to compose.
- [ ] Smoke: `window.confirm` dialogs not driveable by the browse-tool — mark these checks **manual visual** with screenshots.
