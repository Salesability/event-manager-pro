...
- **Tax calculation:** confirmed via the salesability.ca/terms-conditions fetch (2026-05-08) that Salesability Canada Inc. is **Nova Scotia-based** (Dartmouth, NS B2W 6A1; governing-law clause names Nova Scotia courts). **NS HST is 15%, not 13% (ON).** The earlier "hard-code 13%" working assumption is wrong for the seller's province. Two design choices for v1: (a) hard-code 15% based on seller jurisdiction (simpler), or (b) compute by buyer province from the client's mailing address (HST varies — ON 13%, NS/NB/PE/NL 15%, plus GST/PST in non-HST provinces). Whether HST is collected on seller- or buyer-province basis depends on whether services are deemed performed in seller or buyer province (CRA "place-of-supply" rules — likely buyer-province for an on-site event). **Working assumption updated: dealer setting added in 7.3 alongside Stripe so the buyer-province rate applies; until then a quote's `tax` is editable manually by the coach in the line-item editor (no auto-compute in v1).** Confirm with bookkeeping before any real send.
