...
> | **Phase 2** — Shareable Production List | Partial overlap with `docs/chunks/closed/0013-production-export/` (CSV/print) and the abandoned `docs/chunks/closed/0010-calendar-share-full/` work (abandoned 2026-05-11 — per-coach share covers the immediate use case). If the "shareable read-only link" comes back, it warrants a fresh design chunk; 0010's plan body stays in `closed/` as a starting point. |
...
>
> **⚠ Spine settled differently in-app (2026-05-11).** The Phase 3 body below describes a *sequential* flow — Quote signed first, then MSA automatically triggered. The in-app spine locked in `docs/chunks/0037-commercial-spine-msa/` (and described in [`docs/wiki/commercial-spine.md`](../wiki/commercial-spine.md)) instead **bundles the MSA + first Quote into a single Dropbox Sign envelope** for the first deal under a Client, then accepts subsequent Quotes under that active MSA via tokenised public link (no re-signing). The MSA is also explicitly **per-Client and 12-month**, not per-deal. Treat the in-app spine + `commercial-spine.md` as authoritative; the roadmap body stays as a snapshot of how the flow was sketched before the user supplied the full Salesability MSA template.
>
