...
1. **`src/proxy.ts` + `src/lib/supabase/middleware.ts`** — single edge gate, JWT-claim fast-path.
   - Sends unauthenticated requests to `/login?next=<original-path>` (whitelist: `/login`, `/auth/callback`, `/auth/auth-error`, plus public `/share/coach/[id]` per-coach share links and `/book/[token]` customer booking pages — the unguessable per-recipient token is the gate, unknown tokens 404; see [booking.md](booking.md)).
