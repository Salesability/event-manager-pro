...
- **Gate *presence*** (0031, lint-time). A custom ESLint rule (`eslint-plugins/action-gate.mjs` → `action-gate/no-ungated-action`) wired against `src/features/**/actions.ts` (Server Actions) and `src/app/**/route.ts` (Route Handlers). The rule rejects any exported `async` function whose body doesn't reach an allow-listed gate (`assertCan`, `requireRole`, `requireStaffAccess`) — directly or via a same-file wrapper helper that itself calls one. Gate identifier must resolve to an import from `@/lib/auth/*`; member-property name spoofs (`fake.assertCan(...)`), nested-closure gates, and gates inside branching constructs (if/try/switch/while/for/conditional/logical-OR/AND) all fail the rule. The only legitimate exceptions opt out per-function with a `// authz: public` line comment: the auth-flow Server Actions (`signInWithMagicLink`, `signInWithGoogle`, `signOut`) and the OAuth `/auth/callback` GET handler (runs the code-for-session exchange before any session exists).
...
- **Gate *symmetry*** (0034, CI-time). Each `<Can capability="X">` paired with the corresponding `assertCan('X')` server gate.
