...
| `tax_rates` | `id` bigint | `province` enum (`ca_province`, UNIQUE), `label`, `rate` numeric(6,3) (combined GST/HST/PST/QST percent — QC is 14.975), seeded with all 13 provinces (0065). **QuickBooks is the source of truth for `rate` (0075):** the admin **"Pull tax codes"** sync (`src/lib/quickbooks/tax-sync.ts`) **name-matches** each province to the QBO `TaxCode` whose name identifies it ("HST ON" → ON; `resolveProvinceLinksByName`, replacing the 0074 match-by-rate) and **adopts that code's rate** into `tax_rates.rate`. The **in-app rate editor was removed** (`/admin/lookups`; mirrors 0071 making QB the item master) — rates are no longer hand-maintained. Provinces with no confident name match keep their app `rate` as a fallback, **unmanaged**. `quickbooks_tax_code_id` text (nullable, 0074) — the QBO `TaxCode.Id` this province maps to; a province is **"QB-managed" ⇔ this is non-null** (no separate flag column). Set on a confident name match; drives the Estimate push's per-line `TaxCodeRef`. Null = unmapped → a taxed quote on this province fails the push pre-flight. Quote tax still auto-derives from the dealer's province rate (now QB-driven). _Deferred:_ a per-province manual override for ambiguous GST+PST/QST provinces; multi-province adoption is verified on prod (the CA sandbox has only Ontario configured). |
