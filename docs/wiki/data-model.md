...
| `sms_recipients` | `id` bigint | `campaign_id` (FK campaigns, **CASCADE**), `phone` (E.164, CHECK-guarded), `first_name`/`last_name` (nullable), `consent_basis` enum (`sms_consent_basis`: `express\|implied_purchase\|implied_inquiry`), `last_contact_at` date (nullable — implied basis without it = never sendable), `booking_token` (nullable, UNIQUE partial `WHERE NOT NULL` — unguessable ≥18-byte base64url gate for the public `/book/<token>` page; 0108, migration `0054`, see [`booking.md`](booking.md)). UNIQUE `(campaign_id, phone)`; index on `created_at` (import date drives the **24-month retention hard-delete**, `scripts/purge-sms-recipients.ts`). Per-campaign import of the DEALER's list — 0103, see [`sms.md`](sms.md) |
| `campaign_booking_settings` | `id` bigint | `campaign_id` (FK campaigns, **CASCADE**, UNIQUE — row-exists = booking enabled), `day_start_minute`/`day_end_minute` integers (bookable window, minutes from midnight, defaults 540/1020, CHECKs: half-hour aligned, `0 ≤ start < end ≤ 1440`), `slot_capacity` integer (CHECK ≥ 1 — per-campaign, tracks event staffing). Slots are **derived** at read time (30-min constant × campaign days × window) — no slot rows (0108, migration `0054`, see [`booking.md`](booking.md)) |
| `appointments` | `id` bigint | `campaign_id` (FK campaigns, **RESTRICT** — durable customer-intent ledger), `recipient_id` (FK sms_recipients, **SET NULL** — survives the 24-month purge), `slot_date` date + `slot_start_minute` integer (CHECK half-hour aligned — slot identity against the derived grid), `first_name`/`last_name`/`phone` (snapshot at booking time, phone CHECK-guarded), `status` enum (`appointment_status`: `booked\|cancelled` — cancelled frees the seat + the person's one-live-booking allowance). UNIQUE partial `recipient_id WHERE recipient_id IS NOT NULL AND status='booked'` (race backstop); index `(campaign_id, slot_date, slot_start_minute)` for grid counts. Public-booked rows carry null actors (0108, migration `0054`, see [`booking.md`](booking.md)) |
...
| `sms_threads` | `id` bigint | `campaign_id` (FK campaigns, **RESTRICT** — conversation ledger outlives campaign cleanup, and mutable: staff reassignment moves a thread between campaigns), `phone` (E.164, CHECK-guarded), `last_message_at`/`last_inbound_at`/`last_read_at` timestamptz (denormalized by the writers; unread = `last_inbound_at > coalesce(last_read_at, -∞)`; single global read pointer, v1). UNIQUE `(campaign_id, phone)` — a thread is a (campaign, phone) pair. Webhook-created rows carry null actors; staff mutations stamp them (0106, migration `0052`) |
| `sms_thread_messages` | `id` bigint | `thread_id` (FK sms_threads, RESTRICT), `direction` enum (`sms_thread_message_direction`: `inbound\|outbound`), `body` text (unlike `sms_messages`, the BODY lives here — a conversation is unreadable without it), `provider_sid` (UNIQUE partial `WHERE NOT NULL` — inbound replay idempotency), `status` (reuses `sms_message_status`, nullable — CHECK: NULL iff inbound; outbound flipped by the status webhook), `error_code`, `status_updated_at`, `ai_drafted` boolean (provenance: reply originated as an approved AI draft). Inbound webhook-written (null actors); outbound stamps the staff sender (0106) |
...
- `campaigns` 1:* `sms_threads` (RESTRICT) 1:* `sms_thread_messages` (RESTRICT) — the two-way conversation ledger (0106). Threads are keyed UNIQUE `(campaign_id, phone)`; inbound attribution and the console read them per-campaign. No FK to `sms_messages`/`sms_sends` — attribution joins on the `phone` snapshot at webhook time. See [`sms.md`](sms.md).
