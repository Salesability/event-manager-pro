...
| `tax_rates` | `id` bigint | `province` enum (`ca_province`, UNIQUE), `label`, `rate` numeric(6,3) (combined GST/HST/PST/QST percent — QC is 14.975) — admin-editable lookup, seeded with all 13 provinces (0065). Quote tax auto-derives from the dealer's province rate. `quickbooks_tax_code_id` text (nullable, 0074) — the QBO `TaxCode.Id` this province maps to, set by the admin "Pull tax codes" sync (match by rate, unambiguous-only); drives the Estimate push's `TxnTaxDetail.TxnTaxCodeRef`. Null = unmapped → a taxed quote on this province fails the push pre-flight. |
