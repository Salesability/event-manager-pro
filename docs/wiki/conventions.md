...
- **Drizzle** for server SQL — Server Actions, webhook handlers, scripts, transactions. Use it for queries that don't need RLS context (service-role / system writes).
- **`supabase-js`** (via `@supabase/ssr`) for auth, session, RLS-bound reads, and any future realtime. Configured in `src/lib/supabase/`.
- Don't query the same table through both at the same call site. Pick one.
