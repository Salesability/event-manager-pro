...
- **Signing field** — one **BoldSign signature** at the bottom of the Agreement, in the right-column **"For the Client"** box (the Quote-initials field is gone — there's no quote in the envelope). The envelope stays **single-signer**: only the Client signs via BoldSign.
