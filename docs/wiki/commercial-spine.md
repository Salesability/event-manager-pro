...
- **Per-event commercial status.** `loadCommercialStatusByCampaign` (`src/features/schedule/commercial-status.ts`) resolves, per campaign: the latest linked quote's display status + the dealer's active-or-pending MSA + the event's `msa_waived` flag, and **`exposed = !(quote accepted && (MSA active || MSA waived))`** (0100 relaxed the MSA dimension). Batched (2 queries, no N+1); the calendar page passes it to the view.
...
### Event dialog is the workflow hub — deep-linkable + round-trips (0104)

0093 made the event-detail dialog a good *destination* (quote/MSA badges, exposure banner, CTAs); 0104 makes *getting back to it* effortless so a coach never loses their place stepping through **Book Event → Create Quote → Send MSA → Mark Accepted → Protected**. No schema change — pure navigation/context wiring over the existing `quotes.campaign_id` link.

- **The event detail is URL-addressable.** `/calendar?event=<campaignId>` opens that event's detail dialog on load; closing it strips the param (`router.replace('/calendar')`). `CalendarPage` parses `?event=` into `initialEventId` and passes it to `CalendarView`, which opens the matching campaign's dialog once via a ref-latched effect (an unknown/stale id just no-ops — the calendar loads a wide year-1→year+1 range). This deep link is the anchor every step links back to. The key is the **numeric `campaign.id`** (the route is auth-gated; ids already appear in `/quotes/<id>` etc.).
- **The quote step round-trips.** A campaign-scoped `createQuote` now redirects to `/calendar?event=<campaignId>` (was `/quotes/<id>`) so the coach lands back on the event with the quote status updated. The composer shows a persistent **"← [event label]"** link whenever it's campaign-scoped — create-mode tracks the live event-picker selection; edit-mode uses the row's fixed `campaignId` (threaded onto the `Quote` read-model; `/quotes/[id]` loads the linked campaign via `loadCampaign` for the label). **Edit-mode stays put on save** (`setQuoteInputs` keeps its `reset()`+`router.refresh()`) — the link lets the coach choose to return.
- **The MSA step round-trips.** The event-detail **"Send MSA"** CTA carries the event as `/dealerships/<dealerId>?returnEvent=<campaignId>`. The dealer page shows a **"← Back to event"** affordance, and a successful send (`MsaCreateDialog`) routes to `/calendar?event=<returnEvent>` instead of just refreshing — threaded via `returnEventId` through `MsaSendForSignatureButton` → `MsaCreateDialog`. Absent the param, the dialog keeps its prior `router.refresh()` behaviour.
- **The dialog emphasizes the single next step.** `nextCommercialStep(campaignStatus, commercial)` (`src/features/schedule/next-step.ts`, pure + unit-tested, **no new status logic** — derived from the 0093 `CommercialStatus`) picks the one funnel CTA to render as the brand primary: no quote → Create Quote; draft → Edit Quote; quote out but no active/waived MSA → Send MSA; MSA in place + a sent quote waiting → the quote's View/Accept. Precedence follows the accept gate (MSA before accept). That button is the **single** brand primary; the other funnel CTAs and the campaign **Edit** button drop to `outline`, so exactly one primary shows (matches the [[feedback_button_design_system]] "brand blue is the one primary" rule). When the funnel is **complete** (protected — accepted quote + active/waived MSA, so `nextCommercialStep` is `null`), the primary becomes a **"Back to calendar"** done-action (closes the dialog) instead — Edit is never the funnel primary.
- See [`docs/chunks/0104-event-workflow-hub/`](../chunks/closed/0104-event-workflow-hub/intent.md).

### Per-event MSA opt-out (0100)

Not every event wants an MSA — an existing long-term client, a one-off. Before 0100 such an event was permanently painted as an unfinished step (amber "No active MSA", the "⚠ Commercially exposed" nag) **and** its Quote was hard-blocked at the accept gate. 0100 makes the MSA an **opt-out per calendar event** — a coach's call that this event doesn't need one.

- **The flag.** `campaigns.msa_waived boolean NOT NULL DEFAULT false` (migration `0048`). **Per-event only** — the MSA stays a per-client 12-month master agreement ([[project_msa_structure]]); the waiver is an event-level opt-out, not a new MSA scope. Reversible; no backfill (default `false` keeps every existing event unchanged). Threaded onto the `Campaign` projection (`loadCampaigns`/`loadCampaign`).
