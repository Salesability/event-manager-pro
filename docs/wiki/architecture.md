...
1. **Pricing module** — `src/lib/quotes/pricing.ts`. Pure: `computePickedTotals(lines, taxOverride?)` (= `effectiveUnit(line) × qty` summed + typed tax) + `validatePickedLines` + `effectiveUnit(line) = overrideUnitPrice ?? unitPrice`. `MAX_DOLLARS=9_999_999`, `MAX_QTY=1M`; `roundCents` is the single rounding boundary.
...
3. **Composer Server Actions** — `src/features/quotes/actions.ts`, gated `quote:edit`:
   ...
   - `setQuoteTax` / `setQuoteDealer` — tax-override-only / dealer-swap-only setters (unchanged).
