...
- [x] Integration `tests/integration/campaign-delivery.test.ts` (5 cases): calls the real `applyAcceptedQuoteToCampaign` with a tx (injectable `Executor`) inside a rolled-back tx — derive+overwrite+`acceptedQuoteId`; zero-line ⇒ 0; **cross-dealer no-op**; **latest-accepted-wins**; no-campaign no-op. **Verified 5/5 against real Postgres** (local docker test DB, since the sandbox pooler is paused); full integration suite 63/64 there (the 1 fail = `rls.test` role-grant diff, unrelated).
