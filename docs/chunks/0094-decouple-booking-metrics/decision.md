...
- **Cross-dealer write guard.** The campaign UPDATE is scoped to `campaigns.dealerId = quote.dealerId` (not just `campaigns.id = quote.campaignId`). `setQuoteDealer` swaps a draft quote's `dealerId` without reconciling its `campaignId`, so a quote can point at another dealer's campaign; the guard makes a stale cross-dealer link a 0-row no-op instead of an overwrite. The backfill carries the same `dealer_id` guard on **both** the candidate select and the write UPDATE. Surfaced by the chunk-end Codex review; root fix (reconcile `campaignId` on dealer-swap) parked as **0094-c**.
- **Latest-accepted-wins guard.** A campaign can carry more than one accepted quote (revisions). The UPDATE fires only when THIS quote is the latest-accepted for the campaign — a `NOT EXISTS` self-join finds no other accepted quote strictly newer by `(accepted_at, id)`. Closes the concurrent-accept race (two `sent` quotes accepted near-simultaneously snapshotting out of order). The comparison reads both timestamps from the DB (no JS `Date` round-trip), so ordering is exact; ties break on `id`. **Verified against real Postgres** (local docker test DB): older-quote-no-op + newer-quote-wins. Surfaced by the Codex re-review; owner chose to fix in-chunk.
