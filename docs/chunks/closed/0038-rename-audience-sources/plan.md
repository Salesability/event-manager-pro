...
| `docs/chunks/0037-commercial-spine-msa/plan.md` Phase 3 checklist | line listing `salesLeadSourceId` in the `quotes` schema additions | One in-place rename in the plan doc. |
...
- [x] **Leave alone (historical):**
  ...
  - `docs/chunks/closed/**` (closed plans + eval reports — frozen-in-time records).
...
- [x] Edit `docs/chunks/0037-commercial-spine-msa/plan.md` Phase 3 schema-patch checklist (and OQ #7): renamed all `salesLeadSourceId` → `audienceSourceId` and `sales_lead_sources` → `audience_sources` via `sed`. Fixed the historical-context sentence in OQ #7 to read "the lookup (then named `sales_lead_sources`, renamed to `audience_sources` in 0038)" so the funnel-review narrative stays accurate.
- [x] Edit `docs/chunks/closed/0035-quote-composer/plan.md` line 74: updated the `acquired_via` distinct-from reference to `audience_sources` (with the renamed-from-`sales_lead_sources` note).
- [x] Edit `docs/chunks/CURRENT.md` line 6 (forward-looking 0037 description): `salesLeadSourceId` → `audienceSourceId` in the list of commercial columns moving off campaigns. History entries left as-is (they describe past state, where the old name was canonical).
- [ ] **Deferred to a separate close-the-chunk step (per CLAUDE.md):** move `docs/chunks/closed/0038-rename-audience-sources/` → `docs/chunks/closed/0038-rename-audience-sources/`, sweep cross-refs to add `closed/` to the path, flip CURRENT.md Active to 0037 + add a History entry.
