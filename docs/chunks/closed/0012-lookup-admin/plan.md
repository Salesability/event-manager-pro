...
Stub for sub-plan 5.3 of `docs/chunks/0004-port-migration/plan.md`. The legacy app exposes inline ⚙ Manage buttons inside the booking modal (`deprecated/index.html:377` for Event Styles, `:387` for Data Sources) that open dedicated admin modals (`manageStylesModal` lines 522–534, `manageDataSourcesModal` lines 537–549). Each modal is a simple add/remove list against a small lookup table. Done = signed-in users can add, rename, and (soft-)archive `campaign_styles` and `sales_lead_sources` rows from a dedicated UI; the booking form's selects (5.2) reflect changes immediately; existing campaigns referencing an archived lookup row keep rendering their label.
