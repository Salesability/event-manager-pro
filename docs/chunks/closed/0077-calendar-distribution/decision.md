...
>
> **Consequence:** name the **calendar's display name** to the customer brand (recommended **"SaleDay Events"**, matching the "SaleDay Event" titles); keep the technical SA as `eventpro-calendar` (never dealer-visible). The one cosmetic wart is the `c_…@group` email behind the friendly name (visible only on event-detail drill-down). **RESOLVED (2026-06-12):** dealer-facing calendar name = **"EventPro"** — the owner's chosen brand ("EventPro was requested"). The earlier "SaleDay Events" recommendation (to match the `🚗 … — SaleDay Event` titles) was declined; the event **titles keep "SaleDay Event"**, so the organizer ("EventPro") and the titles ("SaleDay Event") intentionally differ. The §2 "warmer branding" sub-point is moot; the rest of §2 (API over `.ics` for real-time overlay + RSVP) still holds.
