...
- **Guided hand-off via "Create quote now?"** — on **Book Event** success the dialog shows a directive next-step (primary **"Create quote now →"**, **"Send MSA"** when the client has no active MSA, quiet **"I'll do this later"**). "Create quote now" opens the **same `QuoteComposer` component as a modal** over the calendar, prefilled with the just-booked event + dealer. We reuse the composer (it's already props-driven) rather than nesting it in the booking form — a quote can't be built inside the booking dialog anyway because it needs the event's id (the event doesn't exist until save). Not a hard block; skipping leaves the event flagged exposed.
- **Quote always scopes to an event** → `quotes.campaignId` is **app-required**. Event-less quote entry points reconcile via **decision C** (a required Event `<select>` *in* the composer, scoped to the chosen dealer) — switched from B (a pre-composer event-step) on 2026-06-24 because the composer already does dealer-selection inline, so the event belongs there too, consistently.
