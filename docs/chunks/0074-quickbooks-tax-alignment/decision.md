...
2. **Txn-level vs line-level `TaxCodeRef`** → **txn-level**: set `TxnTaxDetail.TxnTaxCodeRef = { value: <TaxCode.Id> }` on the Estimate (Canadian manual model). **Drop** the current `TxnTaxDetail.TotalTax` + `GlobalTaxCalculation: TaxExcluded`.
